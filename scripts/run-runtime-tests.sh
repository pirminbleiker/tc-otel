...
#
# Usage:
#   scripts/run-runtime-tests.sh [--profile <softbeckhoff|tc-runtime>] [-- <docker-compose-args>...]
#
# Default profile is softbeckhoff (public CI). The tc-runtime profile requires
# TC_RUNTIME_IMAGE to be set to a user-supplied TwinCAT runtime image.
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --profile)
      PROFILE="${2:-}"
      if [[ -z "$PROFILE" ]]; then
        echo "error: --profile requires a value (softbeckhoff|tc-runtime)" >&2
        exit 2
      fi
      shift 2
      ;;
...
case "$PROFILE" in
  softbeckhoff|tc-runtime) ;;
  *)
    echo "error: unknown profile '$PROFILE' (expected: softbeckhoff|tc-runtime)" >&2
    exit 2
    ;;
esac

if [[ "$PROFILE" == "tc-runtime" && -z "${TC_RUNTIME_IMAGE:-}" ]]; then
  echo "error: --profile tc-runtime requires TC_RUNTIME_IMAGE to be set" >&2
  exit 2
fi
